...
Served over HTTPS it is an installable PWA. On Android/Chrome the page offers an **Install**
button; on iOS, Share → **Add to Home Screen**. It then launches full screen with its own
icon and no browser chrome.

The pieces, all generated by `tools/build.py`: `manifest.webmanifest` (standalone display,
portrait, `#0d1012` theme), `sw.js`, and the icons in `icons/`. The service worker is
**network-first** — a new build is picked up on the next load, and the cache exists only so
the app opens without a connection. Its cache name carries the build timestamp, so old
